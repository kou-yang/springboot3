...
public class CollStreamUtils {
     ...
     * @return 新的Map
     */
    public static <T, K, U> Map<K, U> filterThenToMap(Collection<T> collection, Predicate<? super T> predicate, Function<? super T, ? extends K> keyMapper, Function<? super T, ? extends U> valueMapper, BinaryOperator<U> mergeFunction) {
        return nullDefaultEmpty(collection).stream().filter(predicate).collect(Collectors.toMap(keyMapper, valueMapper, mergeFunction));
    }
